    ...
    addToolbarMenu: function(config) {
        ...
        if (config.group) {
            Y.Array.each(config.items, function (item) {
                if (item.keys) {
                    var itemclass = 'item_' + item.buttonName;
                    if (typeof item.keyDescription !== 'undefined') {
                        // A keyboard shortcut description was specified - use it.
                        this._primaryKeyboardShortcut[itemclass] = item.keyDescription;
                    }
                    this._addKeyboardListener(item.callback, item.keys, itemclass);
    ...
    _showToolbarMenu: function(e, config) {
        ...
        if (!this.menus[config.buttonClass]) {
            ...
            if (config.group) {
                ...
                Y.Array.each(config.items, function (item) {
                    pluginname = item.pluginname ? item.pluginname : pluginname;

                    var title = M.util.get_string(item.title ? item.title : 'pluginname', 'atto_' + pluginname);
                    var node = Y.one('a[class=item_' + item.buttonName + ']');

                    if (!item.iconComponent || item.iconComponent == 'moodle') {
                        item.iconComponent = 'core';
                    }

                    var itemclass = 'item_' + item.buttonName;
                    if (item.keys && this._primaryKeyboardShortcut[itemclass]) {
                        title = M.util.get_string('plugin_title_shortcut', 'editor_atto', {
                            title: title,
                            shortcut: this._primaryKeyboardShortcut[itemclass]
                        });
                    }

                    window.require(['core/templates'], function (Templates) {
                        Templates.renderPix(item.icon, item.iconComponent, title).then(function (iconhtml) {
                            node.one(CSS.ITEMICON).append(iconhtml);
                        });
                    });
                }, this);
            }
